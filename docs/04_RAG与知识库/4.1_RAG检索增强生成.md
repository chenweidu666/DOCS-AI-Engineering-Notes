...
# 2. RAG（Retrieval-Augmented Generation）流程概述
...
## 2.1. 知识库构建阶段（Offline Preparation）

这个阶段是预处理数据，构建可检索的知识库。通常在系统初始化时执行一次，或定期更新。

### 2.1.1. 步骤 1.1: 数据采集和清洗
...
#### 2.5.1.2. 文档增强（Document Enrichment）
...
### 2.5.2. 2 检索阶段优化（Retrieval）

#### 2.5.2.1. 三种检索方式对比（核心概念）
...
#### 2.5.2.4. HyDE（Hypothetical Document Embeddings）
...
#### 2.5.2.6. 关键词 Rerank vs 语义 Rerank
...
## 2.6. RAG 面试问题（补充召回率相关）

以下是常见面试问题，按难度分级，附简要答案提示。

### 2.6.1. 初级问题

#### 2.6.1.1. 什么是 RAG？为什么需要它？
...
#### 2.6.1.2. RAG 和 Fine-Tuning 的区别是什么？
...
#### 2.6.2.1. 在 RAG 中，如何处理查询与文档的不匹配（如同义词问题）？
...
#### 2.6.2.2. 描述一个 RAG 系统的端到端流程，并说明每个步骤的潜在瓶颈。
...
#### 2.6.3.1. 在 RAG 中，如何实现多跳检索（multi-hop retrieval）？
...
#### 2.6.3.2. 如果知识库很大，如何优化 RAG 的检索效率和成本？
...
#### 2.6.3.3. RAG 在处理实时数据（如新闻）时，有什么挑战？如何解决？
...
## 2.9. 📎 参考资料：RAG问答系统设计与实现

# 3. RAG 问答系统 - 基于 PDF 的智能问答工具
