...
## 1.2. LoRA 原理
### 1.2.1. 低秩分解思想
### 1.2.2. rank 与 alpha 参数意义
### 1.2.3. 目标模块选择（Q/K/V/O/FFN）

## 1.3. QLoRA：4bit量化 + LoRA
### 1.3.1. NF4量化
### 1.3.2. 双重量化
### 1.3.3. 分页优化器
...
```
在这里插入图片描述](../图片资源/大模型微调实战指南_示例1.png)
# 5. 实现流程
...
if __name__ == '__main__':
    ...
    inference(model_dir)
```
在这里插入图片描述](../图片资源/大模型微调实战指南_示例2.png)
...
在这里插入图片描述](../图片资源/大模型微调实战指南_示例3.png)
---
