...
## 1.5. 性能调优
### 1.5.1. max_model_len 设置
### 1.5.2. gpu_memory_utilization
### 1.5.3. 量化集成
...
### 2.4.1. 使用 ModelScope 下载 `Qwen2-0.5B-Instruct`
...
### 2.7.1. `ValueError: Invalid repository ID`
- **原因**：模型路径或 ID 错误。
- **解决**：检查模型路径或 ID 是否正确。
...
## 2.9. 📎 参考资料：国产GPU适配vLLM完整指南

# 3. 国产 GPU 适配 vLLM 完整指南
